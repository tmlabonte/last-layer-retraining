...
def finetune_last_layer(
    ...
    if finetune_args.save_self_model:
        finetune_args.check_val_every_n_steps = args.finetune_steps
        finetune_args.ckpt_every_n_steps = args.finetune_steps
    else:
        finetune_args.check_val_every_n_steps = args.finetune_steps + 1
...
def experiment(args, model_class):
    ...
    def finetune_helper(
        ...
        val_worst_group_acc = min([acc for name, acc in val_metrics[0].items() if "group" in name])
    ...
    for finetune_num_data in args.finetune_num_datas:
        for finetune_lr in args.finetune_lrs:
            ...
            finetune_helper2("misclassification self")

            for early_stop_num in args.early_stop_nums:
                finetune_helper2("early-stop misclassification self", early_stop_num=early_stop_num)

            for dropout_prob in args.dropout_probs:
                finetune_helper2("dropout disagreement self", dropout_prob=dropout_prob)

            
            for early_stop_num in args.early_stop_nums:
                finetune_helper2("early-stop disagreement self", early_stop_num=early_stop_num)
